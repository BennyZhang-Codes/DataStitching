function recon_HOOp(HOOp::HighOrderOp{Complex{T}}, Data::AbstractArray{Complex{T},2}, recParams::Dict) where T<:AbstractFloat
...
function recon_HOOp(HOOp::HighOrderOp{Complex{T}}, Data::AbstractArray{Complex{T},2}, weight::AbstractVector{Complex{T}}, recParams::Dict) where T<:AbstractFloat
    recoParams = merge(defaultRecoParams(), recParams)
    reg = Regularization(recParams[:regularization], recParams[:λ]; shape=recParams[:reconSize])
...
function recon_HOOp(HOOp::HighOrderOp_v2{Complex{T}}, Data::AbstractArray{Complex{T},2}, weight::AbstractVector{Complex{T}}, recParams::Dict) where T<:AbstractFloat
    recoParams = merge(defaultRecoParams(), recParams)
    reg = Regularization(recParams[:regularization], recParams[:λ]; shape=recParams[:reconSize])
...
function recon_HOOp(HOOp::HighOrderOp_v1p1, acqData::AcquisitionData, recParams::Dict) :: Matrix
    recoParams = merge(defaultRecoParams(), recParams)
...
function recon_HOOp(HOOp::HighOrderOp_v1, acqData::AcquisitionData, recParams::Dict) :: Matrix
    recoParams = merge(defaultRecoParams(), recParams)
    numContr, numChan = MRIReco.numContrasts(acqData), MRIReco.numChannels(acqData);
    reconSize, weights, L_inv, sparseTrafo, reg, normalize, encOps, solvername, senseMaps = MRIReco.setupIterativeReco(acqData, recParams);
    senseMapsUnCorr = decorrelateSenseMaps(L_inv, senseMaps, numChan);
