export HighOrderOpv2_i2
# recon
using LinearOperators

mutable struct HighOrderOpv2_i2{T,F1,F2} <: AbstractLinearOperator{T}
  nrow :: Int
  ncol :: Int
  symmetric :: Bool
  hermitian :: Bool
...
LinearOperators.storage_type(op::HighOrderOpv2_i2) = typeof(op.Mv5)
...
function HighOrderOpv2_i2(
    ...
    if isnothing(tr_kspha_dt)
        func_prod = (res,xm)->(res .= prod_HighOrderOpv2_i2(xm, bf, nVox, nSam, nCha, tr_kspha, times, fieldmap, csm; Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
    else
        @assert size(tr_kspha_dt) == size(tr_kspha) "tr_kspha_dt must have same size as tr_kspha"
        func_prod = (res,xm)->(res .= prod_dt_HighOrderOpv2_i2(xm, bf, nVox, nSam, nCha, tr_kspha, tr_kspha_dt, times, fieldmap, csm; Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
    end
    func_ctprod = (res,ym)->(res .= ctprod_HighOrderOpv2_i2(ym, bf, nVox, nSam, nCha, tr_kspha, times, fieldmap, csm; Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
    
    return HighOrderOpv2_i2{Complex{T},Nothing,Function}(nRow, nCol, false, false
                , func_prod
                , nothing
                , func_ctprod
...
function prod_dt_HighOrderOpv2_i2(
    ...
    xm = Vector(xm)
    if verbose
        @info "HighOrderOpv2_i2 prod_dt Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
    if use_gpu
        xm = xm |> gpu
...
function prod_HighOrderOpv2_i2(
    xm::AbstractVector{T}, 
    bf::AbstractArray{D, 2},
    nVox::Int64,
    nSam::Int64, 
...
function ctprod_HighOrderOpv2_i2(
    ym::AbstractVector{T}, 
    bf::AbstractArray{D, 2},
    nVox::Int64, 
    nSam::Int64, 
    ...
    if verbose
        @info "HighOrderOpv2_i2 ctprod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
...
function Base.adjoint(op::HighOrderOpv2_i2{T}) where T
