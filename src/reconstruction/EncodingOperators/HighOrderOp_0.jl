export HighOrderOp_0
# recon
using LinearOperators

mutable struct HighOrderOp_0{T,F1,F2} <: AbstractLinearOperator{T}
  nrow :: Int
  ncol :: Int
  symmetric :: Bool
  hermitian :: Bool
...
LinearOperators.storage_type(op::HighOrderOp_0) = typeof(op.Mv5)

"""
    HighOrderOp_0(shape::NTuple{D,Int64}, tr_nominal::Trajectory, tr_measured::Trajectory; Nblocks::Int64=50, use_gpu::Bool=true) where D

# Description
    generates a `HighOrderOp_0` which explicitely evaluates the MRI Fourier HighOrder encoding operator.
...
function HighOrderOp_0(
    ...
    @info "HighOrderOp_0 Nblocks=$Nblocks, use_gpu=$use_gpu, Δx=$Δx, Δy=$Δy"
    @info sim_method
    return HighOrderOp_0{ComplexF64,Nothing,Function}(nrow, ncol, false, false
                , (res,xm)->(res .= prod_HighOrderOp_0(xm, x, y, z, nodes_measured, nodes_nominal, times, fieldmap;
                                        sim_method, Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
                , nothing
                , (res,ym)->(res .= ctprod_HighOrderOp_0(ym, x, y, z, nodes_measured, nodes_nominal, times, fieldmap;
                                        sim_method, Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
                , 0,0,0, false, false, false, ComplexF64[], ComplexF64[])
end

function prod_HighOrderOp_0(
    xm::AbstractVector{T}, 
    x::Vector{Float64}, 
    y::Vector{Float64}, 
    z::Vector{Float64}, 
    ...
    xm = Vector(xm)
    if verbose
        @info "HighOrderOp_0 prod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
...
function ctprod_HighOrderOp_0(
    ...
    xm = Vector(xm)
    if verbose
        @info "HighOrderOp_0 ctprod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
...
function Base.adjoint(op::HighOrderOp_0{T}) where T
