...
"""
mutable struct HighOrderOp_v3{T,F1,F2} <: HOOp{T}
  nrow :: Int
  ncol :: Int
  symmetric :: Bool
  hermitian :: Bool
...
LinearOperators.storage_type(op::HighOrderOp_v3) = typeof(op.Mv5)

"""
    HighOrderOp_v3(grid::Grid{T}, tr_kspha::AbstractArray{T, 2}, times::AbstractVector{T}; fieldmap::Matrix{T} = zeros(T,(grid.nX, grid.nY)), csm::Array{Complex{T}, 3} = ones(Complex{T},(grid.nX, grid.nY)..., 1), sim_method::BlochHighOrder = BlochHighOrder("111"), tr_nominal::AbstractArray{T, 2} = tr_kspha[2:4, :], tr_kspha_dt = nothing, Nblocks::Int64 = 50, use_gpu::Bool = true, verbose::Bool = false)

# Description
    generates a `HighOrderOp_v3` which explicitely evaluates the MRI Fourier HighOrder encoding operator.
...
function HighOrderOp_v3(
    ...
    bf = basisfunc_spha(grid.x, grid.y, grid.z, collect(1:9))
    
    if verbose
        @info "HighOrderOp_v3 Nblocks=$Nblocks, use_gpu=$use_gpu"
    end

    if isnothing(tr_kspha_dt)
        func_prod = (res,xm)->(res .= prod_HighOrderOp_v3(xm, bf, nVox, nSam, nCha, tr_kspha, times, fieldmap, csm; Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
    else
        @assert size(tr_kspha_dt) == size(tr_kspha) "tr_kspha_dt must have same size as tr_kspha"
        func_prod = (res,xm)->(res .= prod_dt_HighOrderOp_v3(xm, bf, nVox, nSam, nCha, tr_kspha, tr_kspha_dt, times, fieldmap, csm; Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
    end
    func_ctprod = (res,ym)->(res .= ctprod_HighOrderOp_v3(ym, bf, nVox, nSam, nCha, tr_kspha, times, fieldmap, csm; Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
    
    return HighOrderOp_v3{Complex{T},Nothing,Function}(nRow, nCol, false, false
                , func_prod
                , nothing
                , func_ctprod
...
function prod_dt_HighOrderOp_v3(
    xm::AbstractVector{T}, 
    ...
    xm = Vector(xm)
    if verbose
        @info "HighOrderOp_v3 prod_dt Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
    if use_gpu
        xm = xm |> gpu
...
function prod_HighOrderOp_v3(
    ...
    xm = Vector(xm)
    if verbose
        @info "HighOrderOp_v3 prod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
    if use_gpu
        xm = xm |> gpu
        out = CUDA.zeros(Complex{D}, nSam, nCha)
...
function ctprod_HighOrderOp_v3(
    ym::AbstractVector{T}, 
    bf::AbstractArray{D, 2},
    nVox::Int64, 
    nSam::Int64, 
    ...
    if verbose
        @info "HighOrderOp_v3 ctprod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
...
function Base.adjoint(op::HighOrderOp_v3{T}) where T
