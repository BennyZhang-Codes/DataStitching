export HighOrderOp_v1p1
# recon
using LinearOperators

mutable struct HighOrderOp_v1p1{T,F1,F2} <: AbstractLinearOperator{T}
  nrow :: Int
  ncol :: Int
  symmetric :: Bool
  hermitian :: Bool
...
LinearOperators.storage_type(op::HighOrderOp_v1p1) = typeof(op.Mv5)

"""
    HighOrderOp_v1p1(reconsize::NTuple{D,Int64}, tr_nominal::Trajectory, tr_measured::Trajectory; Nblocks::Int64=50, use_gpu::Bool=true) where D

# Description
    generates a `HighOrderOp_v1p1` which explicitely evaluates the MRI Fourier HighOrder encoding operator.
...
function HighOrderOp_v1p1(
    ...
    @info "HighOrderOp_v1p1 Nblocks=$Nblocks, use_gpu=$use_gpu, Δx=$Δx, Δy=$Δy"
    @info sim_method
    return HighOrderOp_v1p1{ComplexF64,Nothing,Function}(nRow, nCol, false, false
                , (res,xm)->(res .= prod_HighOrderOp_v1p1(xm, x, y, z, nVox, nSam, nCha, nodes_measured, nodes_nominal, times, fieldmap, csm;
                                        sim_method, Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
                , nothing
                , (res,ym)->(res .= ctprod_HighOrderOp_v1p1(ym, x, y, z, nVox, nSam, nCha, nodes_measured, nodes_nominal, times, fieldmap, csm;
                                        sim_method, Nblocks=Nblocks, parts=parts, use_gpu=use_gpu, verbose=verbose))
                , 0,0,0, false, false, false, ComplexF64[], ComplexF64[])
end

function prod_HighOrderOp_v1p1(
    xm::AbstractVector{T}, 
    x::Vector{Float64}, 
    y::Vector{Float64}, 
    z::Vector{Float64}, 
    ...
    xm = Vector(xm)
    if verbose
        @info "HighOrderOp_v1p1 prod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
...
function ctprod_HighOrderOp_v1p1(
    ym::AbstractVector{T}, 
    x::Vector{Float64}, 
    y::Vector{Float64}, 
    z::Vector{Float64},
    ...
    if verbose
        @info "HighOrderOp_v1p1 ctprod Nblocks=$Nblocks, use_gpu=$use_gpu"
    end
...
function Base.adjoint(op::HighOrderOp_v1p1{T}) where T
