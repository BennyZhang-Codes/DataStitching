...
function demo_PhantomReference(; axis="axial", ss=3, location=0.8, key=:ρ, target_fov=(150, 150), target_resolution=(1,1))
    @assert axis in ["axial", "coronal", "sagittal"] "axis must be axial, coronal, or sagittal"
    @assert key in [:ρ, :T2, :T2s, :T1] "key must be ρ, T2, T2s, or T1"
    @assert 0 <= location <= 1 "location must be between 0 and 1"

    path = "$(dirname(dirname(dirname(@__DIR__))))/$(phantom_dict[:path])/$(phantom_dict[:brain2d])"
