...
TE = 0.0149415; # s
imgs = Array{ComplexF32,3}(undef, Nx, Ny, length(maxOffresonances));
...
imgs[:,:,findall(x->x==maxOffresonance, maxOffresonances)] = abs.(rec.data[:,:]);
end


MAT.matwrite(dir*"/SignalOp_Simu_000_diffB0map.mat", Dict("imgs"=>imgs, "maxOffresonances"=>maxOffresonances))