function [dt, nSeg, nSampleAllSeg, bfield_stitched] = kspha2grad(params)
...
% modified by Jinyuan Zhang, 10/19/2024
    ...
    delay_offset = params.delay_offset; % delay_offset * dt [s], fine tuning of the delay time manually.
    ...
    clear bfield
    delay = gradFreeDelay- scan.k.extTrigDelay + delay_offset - 0.5*dt;  % 0.5*dt for the shift caused by "diff" in "deriveBfieldFromPhase" function
    ...
    for dynamic = 1: nDynamic 
        ...
        for interleave =1 : nInterleave
            for term = 1: nTerm 
                ...
                bfield(:,term,interleave,dynamic) = interp1_TrajTime(bfield(:,term,interleave,dynamic), dt, delay);
            end
        end
    end
