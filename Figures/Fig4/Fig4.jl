...
outpath = "$(@__DIR__)/Figures/Fig4/out"; if ispath(outpath) == false mkpath(outpath) end     # output directory
...
ax5.set_xticks([-800, -500, 0, 500])
ax6.set_xticks([-1500, 0, 1500])
...
for seg = 1:nSegment_fully
    ...
    axs[1,1].plot(t_adc_fully[seg_r], samples_fully_Stitched.h1.A[seg_r]*1e3, color=color_segment[seg], linewidth=linewidth_plot, label="Segment $(seg)")
    axs[1,2].plot(t_adc_fully[seg_r], samples_fully_Stitched.h2.A[seg_r]*1e3, color=color_segment[seg], linewidth=linewidth_plot, label="Segment $(seg)")
end
axs[1,1].plot(t_adc_fully, (samples_fully_Stitched.h1.A-samples_fully_Standard.h1.A)*1e3, color=color_difference, linewidth=linewidth_plot, label="Difference")
axs[1,2].plot(t_adc_fully, (samples_fully_Stitched.h2.A-samples_fully_Standard.h2.A)*1e3, color=color_difference, linewidth=linewidth_plot, label="Difference")
axs[1,3].plot(k_dfc_adc_fully_Stitched[:,2], k_dfc_adc_fully_Stitched[:,3], color=color_gx, linewidth=0.3, label="Stitched")
axs[1,3].plot(k_dfc_adc_fully_Standard[:,2], k_dfc_adc_fully_Standard[:,3], color=color_gy, linewidth=0.3, label="Standard")
...
for seg = 1:nSegment_under
    ...
    axs[2,1].plot(t_adc_under[seg_r], samples_under_Stitched.h1.A[seg_r]*1e3, color=color_segment[color_idx], linewidth=linewidth_plot, label="Stitched")
    axs[2,2].plot(t_adc_under[seg_r], samples_under_Stitched.h2.A[seg_r]*1e3, color=color_segment[color_idx], linewidth=linewidth_plot, label="Stitched")
end 
lDiff_gx, = axs[2,1].plot(t_adc_under, (samples_under_Stitched.h1.A-samples_under_Standard.h1.A)*1e3, color=color_difference, linewidth=linewidth_plot, label="Difference")
lDiff_gy, = axs[2,2].plot(t_adc_under, (samples_under_Stitched.h2.A-samples_under_Standard.h2.A)*1e3, color=color_difference, linewidth=linewidth_plot, label="Difference")
axs[2,3].plot(k_dfc_adc_under_Stitched[:,2], k_dfc_adc_under_Stitched[:,3], color=color_gx, linewidth=linewidth_plot, label="Stitched")
axs[2,3].plot(k_dfc_adc_under_Standard[:,2], k_dfc_adc_under_Standard[:,3], color=color_gy, linewidth=linewidth_plot, label="Standard")
...
for (ax, lDiff) in zip(axs[[2,4]], [lDiff_gx, lDiff_gy])
    ax.legend([Tuple(ls), lines.Line2D([], [], color=color_difference, linewidth=linewidth_legend)], ["Stitched", "Difference"], 
        handler_map=Dict(Tuple(ls) => legend_handler.HandlerTuple(ndivide=length(color_segment), pad=0)),
        fontsize=fontsize_legend, labelcolor=color_label, ncols=2, 
        loc="center left", bbox_to_anchor=(0,1.0),
...
fig.savefig("$(outpath)/Fig4.png", dpi=900, transparent=false, bbox_inches="tight", pad_inches=0.05)
fig.savefig("$(outpath)/Fig4.svg", dpi=900, transparent=false, bbox_inches="tight", pad_inches=0.05)
fig.savefig("$(@__DIR__)/Figures/tiff/Fig4.tiff", dpi=900, transparent=false, bbox_inches="tight", pad_inches=0.05)
