...
Op4 = HighOrderOp((Nx, Ny), tr_nominal, tr_dfc_standard , BlochHighOrder("111"); Δx=Δx, Δy=Δy, Nblocks=Nblocks, fieldmap=Matrix(B0map), grid=1, verbose=true);
...
MAT.matwrite("$(outpath)/R30_snr$(snr)_$(solver)_$(iter)_$(regularization)_$(λ).mat", Dict("imgs"=>imgs, "labels"=>labels, "csm"=>csm, "signal"=>data, "B0map"=>B0map, "x_ref"=>x_ref, "headmask"=>headmask));
