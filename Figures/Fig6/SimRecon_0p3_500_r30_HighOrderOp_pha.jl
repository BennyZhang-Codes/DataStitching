...
coil = csm_Gaussian_grid_block_pha(724, 604, csm_nCoil, true; nRow=csm_nRow, nCol=csm_nCol, nBlock=csm_nBlock, relative_radius=csm_radius, verbose=true);
...
Nblocks = 50;   # the number is set according to the GPU memory.
use_gpu = true;
verbose = false;
...
MAT.matwrite("$(outpath)/0p3_500_r30_pha_snr$(snr)_$(solver)_$(iter)_$(regularization)_$(λ).mat", results)


for idx in eachindex(Ops)
    @info "$(labels[idx]), SSIM: $(HO_SSIM(x_ref, abs.(imgs[:, :, idx])))"
    HO_img_scale(x_ref, abs.(imgs[:, :, 4]))
end
plt_image(x_ref, vmax=1, vmin=0)
plt_image(HO_img_scale(x_ref, abs.(imgs[:, :, 3])), vmax=1, vmin=0)
plt_image(HO_img_scale(x_ref, abs.(imgs[:, :, 4])), vmax=1, vmin=0)

plt_image(abs.(x_ref - HO_img_scale(x_ref, abs.(imgs[:, :, 3]))), vmax=1, vmin=0)
...
HOOpStitched = HighOrderOp(gridding, T.(kspha_stitched[:, 1:9]'), T.(datatime); sim_method=BHO, tr_nominal=T.(kspha_nominal'), 
                        Nblocks=Nblocks, fieldmap=T.(b0), csm=Complex{T}.(csm), use_gpu=use_gpu, verbose=verbose);

HOOpStandard = HighOrderOp(gridding, T.(kspha_standard[:, 1:9]'), T.(datatime); sim_method=BHO, tr_nominal=T.(kspha_nominal'), 
                        Nblocks=Nblocks, fieldmap=T.(b0), csm=Complex{T}.(csm), use_gpu=use_gpu, verbose=verbose);

y          = HOOpStitched * vec(im_grid);
xStitched = reshape(HOOpStitched' * y, nX, nY);
xStandard = reshape(HOOpStandard' * y, nX, nY);
plt_image(abs.(xStitched), vmaxp=99.9)
plt_image(abs.(xStandard), vmaxp=99.9)
...
@time imStitched = recon_HOOp(HOOpStitched, Complex{T}.(reshape(y, nSample, nCha)), Complex{T}.(weight), recParams);
plt_image(abs.(imStitched); vmin=0, vmax=1, title="HOOp Stitched")
println("SSIM: ", HO_SSIM(abs.(im_grid), abs.(imStitched)))

@time imStandard = recon_HOOp(HOOpStandard, Complex{T}.(reshape(y, nSample, nCha)), Complex{T}.(weight), recParams);
plt_image(abs.(imStandard); vmin=0, vmax=1, title="HOOp Standard")
println("SSIM: ", HO_SSIM(abs.(im_grid), abs.(imStandard)))

@time imStitched = recon_HOOp(HOOpStitched, Complex{T}.(reshape(y, nSample, nCha)), recParams);
plt_image(abs.(imStitched); vmin=0, vmax=1, title="HOOp Stitched")
println("SSIM: ", HO_SSIM(abs.(im_grid), abs.(imStitched)))

@time imStandard = recon_HOOp(HOOpStandard, Complex{T}.(reshape(y, nSample, nCha)), recParams);
plt_image(abs.(imStandard); vmin=0, vmax=1, title="HOOp Standard")
println("SSIM: ", HO_SSIM(abs.(im_grid), abs.(imStandard)))
