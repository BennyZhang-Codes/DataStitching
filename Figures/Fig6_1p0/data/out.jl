...
out_path     = "$(path)/data" ; if ispath(out_path) == false mkpath(out_path) end
...
for idx = 1 : nCha
    fig = plt_image(abs.(csm[:,:,idx]); height=fig_height, width=fig_width, vmax=vmax, vmin=vmin)
    fig.savefig("$(out_path)/csm_Birdcage_cha$(idx).png", dpi=900, transparent=true, bbox_inches="tight", pad_inches=0)
end
fig = plt_images(  abs.(csm); dim=3, nRow=2, nCol=8, vmax=vmax, vmin=vmin)
fig.savefig("$(out_path)/csm_Birdcage_mag.png", dpi=900, transparent=true, bbox_inches="tight", pad_inches=0)
fig = plt_images(angle.(csm); dim=3, nRow=2, nCol=8, vmax=π, vmin=-π)
fig.savefig("$(out_path)/csm_Birdcage_pha.png", dpi=900, transparent=true, bbox_inches="tight", pad_inches=0)
