...
skope = matread("$(path)/Spiral_2p0x2p0x2p0_100x100x35_1x1_int1_9p5ms_tr500_te5p0_DFC_k_g.mat")
...
while iblock <= nBlock
    if seq.GR[1, iblock].delay > 0 && is_ADC_on(seq[iblock])
        g = InterpTrajTime(g_skope[iadc, :, :], gradRasterTime, -0.7*gradRasterTime, t_g)
        ...
        for i = 1:16
            ...
            hoseq.GR_dfc[i, iblock].T    = (nGrad_delay+nGrad_adc-1)*gradRasterTime;
            hoseq.GR_dfc[i, iblock].A    = g[nGrad_Gz_pre+1:end, i];
            hoseq.GR_dfc[i, iblock].rise = gradRasterTime/2;
            hoseq.GR_dfc[i, iblock].fall = gradRasterTime/2;
            if i == 2 || i == 3
                hoseq.GR_dfc[i, iblock].T = (nGrad_delay+nGrad_adc+nGrad_Spoiler-1)*gradRasterTime;
                hoseq.GR_dfc[i, iblock].A = [g[nGrad_Gz_pre+1:end, i]; seq.GR[i-1, iblock].A[end-nGrad_Spoiler+1:end]]
            end
        end
        iadc += 1
    end
