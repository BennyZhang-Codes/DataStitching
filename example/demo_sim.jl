...
# 1. hoseq
seq = demo_seq(seqname="spiral", r=1)
hoseq = HO_Sequence(seq)
plot_kspace(seq)
# plot_hoseqd(hoseq);

# 2. phantom
obj = brain_phantom2D(BrainPhantom(); ss=5, location=0.8, csmtype=:fan, nCoil=1); info(obj)
obj.Δw .= obj.Δw * 0; # γ*1.5*(-3.45)*1e-6 * 2π  cancel Δw
obj.T2 .= obj.T2 * Inf;   # cancel T2 relaxiation
plot_phantom_map(obj, :Dθ; darkmode=true)

# ref = brain_phantom2D_reference(BrainPhantom(); B0map=:quadratic,key=:Δw, maxOffresonance=maxOffresonance); 
...
# 2. phantom
# maxOffresonance = 20.
obj = brain_phantom2D(BrainPhantom(); ss=3, location=0.8, B0map=:file, maxOffresonance=maxOffresonance); info(obj)
obj.Δw .= obj.Δw * 0; # γ*1.5*(-3.45)*1e-6 * 2π  cancel Δw
# obj.T2 .= obj.T2 * Inf;   # cancel T2 relaxiation
p_Δw = plot_phantom_map(obj, :Δw; darkmode=true)
# savefig(p_Δw, (@__DIR__)*"/brain2D_B0map_Δw.svg", width=500,height=500,format="svg")
ref = brain_phantom2D_reference(BrainPhantom(); B0map=:file,key=:Δw, maxOffresonance=maxOffresonance); 
