...
csmtype  = :rect; nCoil = 25; nrows=5; ncols=5;
BHO_name = "000"
folder   = "gre_$(csmtype)_nCoil$(nCoil)_$(nrows)x$(ncols)"
path     = "$(@__DIR__)/rect_sense/gre/$folder"; if ispath(path) == false mkpath(path) end

h = nrows*100; w = ncols*100;
...
raw = signal_to_raw_data(signal, hoseq)
...
PlotlyJS.savefig(p_sos   , "$(path)/$(filename)-ifft_SOS.svg", format="svg", height=400, width=450)
PlotlyJS.savefig(p_images, "$(path)/$(filename)-ifft.svg"    , format="svg", height=h, width=w)
...
PlotlyJS.savefig(p_traj, "$(path)/$(filename)_R$(r)-traj.svg", format="svg", height=400, width=400)
...
PlotlyJS.savefig(p_smap_mag , "$(path)/$(filename)_simSmap_mag.svg"        , format="svg", height=h, width=w)
PlotlyJS.savefig(p_smap_pha , "$(path)/$(filename)_simSmap_pha.svg"        , format="svg", height=h, width=w)
PlotlyJS.savefig(p_smap_sos , "$(path)/$(filename)_simSmap_sos.svg"        , format="svg", height=400, width=450)
PlotlyJS.savefig(p_img_recon, "$(path)/$(filename)_R$(r)-simSmap_Sense.svg", format="svg", height=400, width=450)
