...
# Setup
############################################################################################## 
simtype = SimType(B0=true, T2=false, ss=1)                       # turn on B0, turn off T2, set phantom subsampling to 5
...
#############################################################################
# SENSE
solver = "admm"; regularization = "TV"; iter = 30; λ = 1e-4;
solver = "cgnr"; regularization = "L2"; iter = 90; λ = 1e-3;
...
fig = plt_image(rotl90(abs.(rec)); vminp=0, vmaxp=99.9)  #plt.imshow(rotl90(abs.(rec)), cmap="gray", interpolation="spline36")

# HO_SSIM(rotl90(abs.(rec)), rotl90(x_ref))
# a, b = rotl90(abs.(rec)), rotl90(x_ref);
# a = HO_img_scale(rotl90(x_ref), rotl90(abs.(rec)))

# SENSE with HighOrderOp
...
# Op7 = HighOrderOp((Nx, Ny), tr_nominal, tr_dfc_stitched , BlochHighOrder("000"); Nblocks=10, fieldmap=Matrix(B0map), grid=1, verbose=true);
...
Op4 = HighOrderOp((Nx, Ny), tr_nominal, tr_dfc_standard , BlochHighOrder("111"); Nblocks=Nblocks, fieldmap=Matrix(B0map), grid=1);
...
MAT.matwrite(dir*"/fully_0.2_wonoise_$(solver)_$(iter)_$(regularization)_$(λ).mat", 
