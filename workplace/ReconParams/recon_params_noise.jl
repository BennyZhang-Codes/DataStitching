...
Op1 = HighOrderOp_v1((Nx, Ny), tr_nominal, tr_dfc_stitched , BlochHighOrder("000"); Nblocks=9, fieldmap=Matrix(B0map).*0, grid=1);
# 4. include both ΔB₀ and all order terms of stitched DFC, with BlochHighOrder("111")
Op4 = HighOrderOp_v1((Nx, Ny), tr_nominal, tr_dfc_stitched , BlochHighOrder("111"); Nblocks=9, fieldmap=Matrix(B0map), grid=1);
