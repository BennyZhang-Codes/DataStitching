...
figure, hold on;
for seg = 1: nSeg
    plot(kspha(:,2,1,seg), kspha(:,3,1,seg));
end
...
shift = mean(kspha_shift(nSamplePerSeg+1:nSamplePerSeg+100,2,1,3) - kspha_shift(1:100,2,1,4));
plot(kspha_shift(nSamplePerSeg+1:nSample,2,1,3));
plot(kspha_shift(1:nSample-nSamplePerSeg,2,1,4)+shift);
subplot(1,2,2)
plot(kspha_shift(nSamplePerSeg+1:nSample,2,1,3)-kspha_shift(1:nSample-nSamplePerSeg,2,1,4)-shift);
...
plot_kspha(diff(kspha_stitched) / 1e-9 / 2/pi/ mr.opts().gamma - bfield_stitched,dt)
...
figure, subplot(2,1,1), hold on;
plot(bfield_kspha_stitched(:, 2));   % Hz/m
plot(bfield_stitched(1:end-1, 2));
subplot(2,1,2);
plot(bfield_stitched(1:end-1, 2)-bfield_kspha_stitched(:, 2));
% 
%% comparison of bfield_kspha_stitched and bfield_stitched, both of them were converted to traj
k_kspha  = grad2traj(bfield_kspha_stitched(1:end,:), dt);
k_bfield = grad2traj(bfield_stitched(1:end-1,:), dt);
...
figure,hold on;
plot(k_kspha(:, 2), k_kspha(:, 3));            % 1/m
plot(k_bfield(1:end, 2), k_bfield(1:end, 3));

%% 

load('xw_sp2d-1mm-r4.mat')
gx = interp1(1:2951, grad0.shape(1, :), 0.1*(1:29510), 'makima',0);
gy = interp1(1:2951, grad0.shape(2, :), 0.1*(1:29510), 'makima',0);
...
% legend("Nominal", "FilterAfterStitch", "FilterBeforeStitch");

%%
k = grad2traj(skopeStitched,dt);
% plot_kspha(k, dt);
figure, hold on;
% plot(kx,ky);
plot(bfield_stitched(:, 2), bfield_stitched(:, 3));            % 1/m
plot(bfield_kspha_stitched(:, 2), bfield_kspha_stitched(:, 3));            % 1/m