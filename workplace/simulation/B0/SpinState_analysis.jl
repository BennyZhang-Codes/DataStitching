...
# hoseq_base = HO_Sequence(load_seq(seqname="spiral", r=1))

dir = "$(@__DIR__)/B0/results/spinstate/$(hoseq_base.SEQ.DEF["Name"])/"; if ispath(dir) == false mkpath(dir) end
...
hoseq_dict = Dict(
    ...
    :fatsatw     =>hoseq_base[2:end],
    :fatsatwo    =>hoseq_base[4:end],
    )
for maxOffresonance in [0.,100., 200., 300., 400., 1000., 1500.]
# for key in keys(hoseq_dict)
# for key in [:fatsatw, :fatsatwo]
