sampling_params=KomaMRIBase.default_sampling_params()
hoseqd = HO_discretize(hoseq; sampling_params)

Gx = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.seqd.Gx*1e3, name="Gx", mode="markers+lines", marker_symbol=:circle)
Gy = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.seqd.Gy*1e3, name="Gy", mode="markers+lines", marker_symbol=:circle)
Gz = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.seqd.Gz*1e3, name="Gz", mode="markers+lines", marker_symbol=:circle)
B1 = scatter(x=hoseqd.seqd.t*1e3, y=abs.(hoseqd.seqd.B1*1e6), name="|B1|", mode="markers+lines", marker_symbol=:circle)
ADC = scatter(x=hoseqd.seqd.t[hoseqd.seqd.ADC]*1e3, y=zeros(sum(hoseqd.seqd.ADC)), name="ADC", mode="markers", marker_symbol=:x)

h0 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h0*1e3, name="h0", mode="markers+lines", marker_symbol=:circle)
h1 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h1*1e3, name="h1", mode="markers+lines", marker_symbol=:circle)
h2 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h2*1e3, name="h2", mode="markers+lines", marker_symbol=:circle)
h3 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h3*1e3, name="h3", mode="markers+lines", marker_symbol=:circle)
h4 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h4*1e3, name="h4", mode="markers+lines", marker_symbol=:circle)
h5 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h5*1e3, name="h5", mode="markers+lines", marker_symbol=:circle)
h6 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h6*1e3, name="h6", mode="markers+lines", marker_symbol=:circle)
h7 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h7*1e3, name="h7", mode="markers+lines", marker_symbol=:circle)
h8 = scatter(x=hoseqd.seqd.t*1e3, y=hoseqd.h8*1e3, name="h8", mode="markers+lines", marker_symbol=:circle)

Gx = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.seqd.Gx*1e3, name="Gx", mode="markers+lines", marker_symbol=:circle)
Gy = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.seqd.Gy*1e3, name="Gy", mode="markers+lines", marker_symbol=:circle)
Gz = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.seqd.Gz*1e3, name="Gz", mode="markers+lines", marker_symbol=:circle)
B1 = scattergl(x=hoseqd.seqd.t*1e3, y=abs.(hoseqd.seqd.B1*1e6), name="|B1|", mode="markers+lines", marker_symbol=:circle)
ADC = scattergl(x=hoseqd.seqd.t[hoseqd.seqd.ADC]*1e3, y=zeros(sum(hoseqd.seqd.ADC)), name="ADC", mode="markers", marker_symbol=:x)
h0 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h0*1e3, name="h0", mode="markers+lines", marker_symbol=:circle)
h1 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h1*1e3, name="h1", mode="markers+lines", marker_symbol=:circle)
h2 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h2*1e3, name="h2", mode="markers+lines", marker_symbol=:circle)
h3 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h3*1e3, name="h3", mode="markers+lines", marker_symbol=:circle)
h4 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h4*1e3, name="h4", mode="markers+lines", marker_symbol=:circle)
h5 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h5*1e3, name="h5", mode="markers+lines", marker_symbol=:circle)
h6 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h6*1e3, name="h6", mode="markers+lines", marker_symbol=:circle)
h7 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h7*1e3, name="h7", mode="markers+lines", marker_symbol=:circle)
h8 = scattergl(x=hoseqd.seqd.t*1e3, y=hoseqd.h8*1e3, name="h8", mode="markers+lines", marker_symbol=:circle)

KomaMRIPlots.plot_koma([Gx,Gy,Gz,B1,ADC,h0,h1,h2,h3,h4,h5,h6,h7,h8])

